...
export type BossKind = "darklord" | "muska" | "god"
...
export function isBossStage(s: Stage): boolean {
  return s === "stage3" || s === "stage6" || s === "stage9"
}
...
export function hasFireRainUnlocked(s: Stage): boolean {
  return s === "stage2" || s === "stage3" || s === "stage4" || s === "stage5" || s === "stage6" || s === "stage7" || s === "stage8" || s === "stage9"
}

export function hasFireStormUnlocked(s: Stage): boolean {
  return s === "stage3" || s === "stage4" || s === "stage5" || s === "stage6" || s === "stage7" || s === "stage8" || s === "stage9"
}

export function hasCometUnlocked(s: Stage): boolean {
  return s === "stage5" || s === "stage6" || s === "stage7" || s === "stage8" || s === "stage9"
}
...
export function useGameEngine(canvasRef: React.RefObject<HTMLCanvasElement | null>) {
  ...
  function updatePlayer(gs: GameState, inp: InputState, now: number) {
    ...
    if (inp.firePressed && gs.skillCooldown <= 0 && p.energy >= 1 && p.castTimer <= 0) {
      ...
      p.castTimer = 12
      p.state = "cast"
      // Fireball emerges from the wizard's staff tip
      const fbX = p.facing === 1 ? p.x + 22 : p.x - 12
      const fbY = p.y + 10
      const wl = gs.weaponLevel
      gs.fireballs.push({
        id: nextId(), x: fbX, y: fbY,
        w: 14 + (wl - 1) * 4, h: 12 + (wl - 1) * 2,
  ...
  function updateEnemies(gs: GameState, now: number) {
    ...
    for (const e of gs.enemies) {
      ...
      if (e.type === "boss" && e.phase === 1 && e.hp <= e.maxHp / 2) {
        e.phase = 2
        gs.screenShake = 12
        const name = e.bossKind === "muska" ? "MUSKA ENRAGED" : e.bossKind === "god" ? "GOD AWAKENS" : "PHASE 2!"
        ...
        if (dist < e.aggroRange && p.state !== "dead") {
          ...
          if (e.type === "boss" && e.bossProjectileCooldown !== undefined) {
            ...
            if (e.bossProjectileCooldown <= 0) {
              const baseShots = e.bossKind === "muska" ? 3 : e.bossKind === "god" ? 3 : 1
              const shots = e.phase === 2 ? baseShots + 2 : baseShots
              const spd = e.bossKind === "muska" ? 6 : 4.5
              const w = e.bossKind === "muska" ? 8 : 12
              const h = e.bossKind === "muska" ? 6 : 12
  ...
  function startEnemySpecial(gs: GameState, e: Enemy) {
    ...
    e.specialTelegraph = label === "FEATHER STORM!" ? 30 : duration
    e.specialCooldown = cooldown
    const ex = e.x + e.w / 2
    const ey = e.y + e.h / 2

    if (label === "FEATHER STORM!") {
      featherRain(gs, e)
    } else {
      gs.aoeEffects.push({
        id: nextId(),
  ...
  function shootEnemyProjectile(gs: GameState, e: Enemy) {
    ...
    const spd = e.type === "robotElite" ? 4 : e.type === "seraph" ? 3.5 : 3
    const bulletType = e.type === "robot" || e.type === "robotElite" ? "robotBullet" : "enemy"
    for (let i = 0; i < shots; i++) {
...
function draw(canvas: HTMLCanvasElement | null, gs: GameState) {
  ...
  // Background
  drawBackground(ctx, gs.stage, cam, gs.stageWidth)
...
function drawBackground(ctx: CanvasRenderingContext2D, stage: Stage, cam: number, stageWidth: number) {
...
function drawPlayer(ctx: CanvasRenderingContext2D, p: Player, cam: number, frame: number) {
  ...
  let frameIndex = 0
  if (p.state === "cast") {
    // Cast animation plays frames 6-11 over the cast duration
    const castProgress = 1 - Math.min(1, p.castTimer / 36)
    frameIndex = 6 + Math.min(5, Math.floor(castProgress * 6))
  ...
  // Draw wizard sprite aligned so feet sit on the hitbox bottom
  const drawX = sx + p.w / 2 - WIZARD_SPRITE_W / 2
  const drawY = p.y + p.h - WIZARD_SPRITE_H
...
function drawEnemy(ctx: CanvasRenderingContext2D, e: Enemy, cam: number) {
  ...
  const hover = (e.type === "angel" || e.type === "seraph") ? Math.sin(e.animFrame * 0.6) * 3 : 0
  ctx.save()
